...
func _emit_next_char() -> void:
	...
	if is_space(data.char) or data.char in WORD_BREAK_CHARS:
		_dynamic_delay_ms = base_delay_ms
	else:
...
func _consume_char_for_word_state(data: Dictionary, emit_signals: bool) -> void:
	...
	if not is_space(ch) and not (ch in WORD_BREAK_CHARS):
		_word_buffer += ch
...
static func is_space(ch: String) -> bool:
	if ch.is_empty():
		return false
	var cp := ch.unicode_at(0)
	# ASCII 공백류
	if cp == 0x20 or cp == 0x09 or cp == 0x0A or cp == 0x0B or cp == 0x0C or cp == 0x0D:
		return true
	# 유니코드 공백 단일 코드포인트
	if cp == 0x00A0 or cp == 0x1680 or cp == 0x2028 or cp == 0x2029 or cp == 0x202F or cp == 0x205F or cp == 0x3000 or cp == 0xFEFF:
		return true
	# 0x2000..0x200A (en/em space 등)
